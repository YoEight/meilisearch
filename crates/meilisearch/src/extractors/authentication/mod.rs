        ...
        match req.app_data::<Data<AuthController>>().cloned() {
            Some(auth) => match extract_token_from_request(req) {
                Ok(Some(token)) => {
                    ...
                    // TODO: find a less hardcoded way?
                    Box::pin(async move {
                        let guarded = GuardedData::<P, D>::auth_bearer(
                            auth,
                            token,
                            index,
                            Some(data),
                        ...
                        Ok(OptionallyGuardedData { data: guarded.data, filters: guarded.filters, _marker: PhantomData })
                    })
                }

                Ok(None) => Box::pin(ok(OptionallyGuardedData { data, filters: AuthFilter::default(), _marker: PhantomData })),
                Err(e) => Box::pin(err(e.into())),
            },
